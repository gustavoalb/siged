...
def alerta_a_preencher(escola,d)
 if escola.calcular_a_preencher(d) > 0 and escola.calcular_oferta(d) < escola.calcular_a_preencher(d)
  return raw("<b style='color: red;'>#{escola.calcular_a_preencher(d)}</b>")
else
  return escola.calcular_a_preencher(d)
end
end
end
